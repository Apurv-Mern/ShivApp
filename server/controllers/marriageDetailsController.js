...
const createMarriageDetail = async (req, res) => {
    try {
        const { bride_name, groom_name, bride_fathers_name, bride_mothers_name, groom_fathers_name, groom_mothers_name, user_id } = req.body;
        ...
        const values = [bride_name, groom_name, bride_fathers_name, bride_mothers_name, groom_fathers_name, groom_mothers_name, user_id];
  ...
  const getMarriageDetailsByUser = async (req, res) => {
    ...
    if(!user_id) return res.status(400).json({"Msg":"null user id provided"});
  try {
    const query = `
      SELECT *
  ...
  const  updateMarriageDetail = async (req, res) => {
    ...
    if(!user_id) return res.status(400).json({"Msg":"null user id provided"});
    const { bride_name, groom_name, bride_fathers_name, bride_mothers_name, groom_fathers_name, groom_mothers_name } = req.body;
    try {
  ...
  const deleteMarriageDetail = async (req, res) => {
    const { id } = req.params;
    if(!id) return res.status(400).json({"Msg":"null user id provided"});
    try {

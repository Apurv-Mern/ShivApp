...
router.post("/signup", async (req, res) => {
  ...
  if (valid_number.rows[0] || number.length > 10) {
    return res.status(400).end("Mobile number already exists.");
  }
  try {
    ...
    if (users) {
      ...
      //signup
       try {
         ...
         pool.query(
           "INSERT INTO users (username,password, email, number, date_of_birth,profile_photo) VALUES ($1,$2,$3,$4,$5,$6) RETURNING *",
           [name, hashedPassword, email, number, dob,data.photo],
           async (error, results) => {
             if (error) {
               res.status(401).json({ error: error });
               throw error;
